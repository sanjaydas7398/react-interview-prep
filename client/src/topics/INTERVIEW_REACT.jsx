...
import {
  addReactCodingQuestion, updateReactCodingQuestion, deleteReactCodingQuestion,
  addReactScenarioQuestion, updateReactScenarioQuestion, deleteReactScenarioQuestion
} from "../api";
...
function CodeEditor({ value, onChange }) {
  ...
  const myTheme = EditorView.theme({
    ".cm-content": { color: "#f5f5f5", background: "rgba(255,255,255,0.05)", fontFamily: "'Fira Code', monospace", fontSize: "14px" },
    ".cm-gutters": { background: "rgba(255,255,255,0.05)", color: "#888" },
    ".cm-activeLine": { background: "rgba(255,255,255,0.08)" },
  }, { dark: true });
...
<body>
  ...
  <script type="text/babel" data-presets="react">
    const { useState, useEffect, useRef, useReducer, useCallback, useMemo, useContext } = React;
    ${code}
        ...
        {filtered.map((q, i) => (
          <div key={q.id} style={{ padding: "24px", borderBottom: i === filtered.length - 1 ? "none" : `1px solid ${C.border}`, display: "flex", alignItems: "flex-start", gap: 20, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = C.bgHover} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            ...
            <div style={{ flex: 1 }} onClick={() => { if (activeTab === "Machine Coding") { setSelectedCoding(q); setUserCode(q.templateCode); setPreviewCode(q.templateCode); setRunKey(k => k + 1); setEditorTab("description"); setRevealAnswer(false); } else { setSelectedScenario(q); setRevealAnswer(false); } }}>
              <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, color: C.text, display: "flex", alignItems: "center", gap: 8 }}>{q.title || q.description.split("\n")[0]} {q.isMostAsked && <span style={{ fontSize: 11, fontWeight: 700, background: "#fef9c3", color: "#92400e", padding: "2px 8px", borderRadius: 20, border: "1px solid #fde68a" }}>⭐ Most Asked</span>}</div>
              {q.title && <div style={{ fontSize: 14, color: C.textMuted, marginBottom: 16 }}>{q.description.split("\n")[0]}</div>}
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: C.textMuted }}><span style={{ padding: "2px 6px", background: C.bgPanel, borderRadius: 4, fontWeight: 600 }}>REACT</span> {q.category || "React Scenario"}</div>
            </div>
          ...
          <div style={{ width: "100%", maxWidth: 800, maxHeight: "85vh", background: C.bgCard, borderRadius: 24, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "32px" }}>
              ...
              {!revealAnswer ? (
                ...
                <div><h4 style={{ fontSize: 13, textTransform: "uppercase", color: "#00c9a7", marginBottom: 12 }}>Answer</h4><div className="quill-content" style={{ fontSize: 15, color: C.text, lineHeight: 1.7, background: "#007a5510", padding: 24, borderRadius: 16, border: "1px solid #007a5533" }} dangerouslySetInnerHTML={{ __html: selectedScenario.explanation }}></div></div>
              )}
            </div>
          </div>
        </div>
